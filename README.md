...
1. An Authority
	...
	Before we start the authority, we need to generate the databases with the precomputed values.
	The databases are generated by the script found in the "tools" directory. Their total size is ~120MB.
	This procedure needs to be done only once, and it is safe to copy the generated files if more than one dbs are used.
